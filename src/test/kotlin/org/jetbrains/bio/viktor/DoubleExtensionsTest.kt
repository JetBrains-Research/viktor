...
class DoubleExtensionsTest {
    @Test fun plusVector() {
        val v = F64Vector(10) { it.toDouble() }
        val incremented = F64Vector(10) { it + 1.0 }
        assertEquals(incremented, 1.0 + v)
    }

    @Test fun plusMatrix() {
        val m = F64Matrix(10, 2) { i, j -> i + 2.0 * j }
        val incremented = F64Matrix(10, 2) { i, j -> m.ix[i, j] + 1.0 }
    ...
    @Test fun minusMatrix() {
        val m = F64Matrix(10, 2) { i, j -> i + 2.0 * j }
        val decremented = F64Matrix(10, 2) { i, j -> 42.0 - m.ix[i, j] }
        assertEquals(decremented, 42.0 - m)
    }

    @Test fun timesVector() {
        val v = F64Vector(10) { it.toDouble() }
        val scaled = F64Vector(10) { it * 42.0 }
    ...
    @Test fun divVector() {
        val v = F64Vector(10) { it.toDouble() }
        val scaled = F64Vector(10) { 1.0 / it }
